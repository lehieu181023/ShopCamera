...
function BlockUI(message, element) {
    var text = message || "Loading...";
    var $target = element ? $(element) : $("body"); // nếu không có element → block toàn trang

    $target.LoadingOverlay("show", {
        text: text,
        spinner: true,            // dùng spinner mặc định
        spinnerSize: "large",         // to hơn
        spinnerColor: "#000",          // màu spinner
        background: "rgba(255,255,255,0.6)" // nền overlay
    });
}

// Hàm Unblock UI
function UnBlockUI(element) {
    var $target = element ? $(element) : $("body");
...
// Confirm xóa dạng SweetAlert2
function confirmDelete(message = 'Bạn có chắc chắn muốn xóa không?', callback) {
    Swal.fire({
        title: 'Xác nhận',
        text: message,
        icon: 'warning',
...
function initSelect2(context) {
    // context = phạm vi tìm (mặc định là document)
    context = context || document;

    $(context).find('select').each(function () {
        var $this = $(this);

        // Nếu đã có select2 hoặc có class notSelect2 thì bỏ qua
        if ($this.hasClass('select2-hidden-accessible') || $this.hasClass('notSelect2')) {
            return; // skip
        }
        ...
        if ($modalParent.length) {
            options.dropdownParent = $modalParent; // Nếu nằm trong modal
        }
...
$(document).ready(function () {
    ...
    // Init Select2 cho các select xuất hiện sau (ví dụ modal)
